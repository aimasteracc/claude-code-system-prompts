<!--
name: 'Tool Description: AskUserQuestion (preview field)'
description: Instructions for using the HTML preview field on single-select question options to display visual artifacts like UI mockups, code snippets, and diagrams
ccVersion: 2.1.69
-->

预览功能：
在呈现用户需要视觉比较的具体成果时，请在选项中使用可选的 `preview` 字段：
- UI 布局或组件的 HTML 模拟图
- 展示不同实现方式的格式化代码片段
- 视觉比较或图表

预览内容必须是独立的 HTML 片段（无 <html>/<body> 包装，无 <script> 或 <style> 标签——改用内联 style 属性）。不要将预览用于仅凭标签和描述就足够的简单偏好问题。注意：预览仅支持单选问题（不支持 multiSelect）。
